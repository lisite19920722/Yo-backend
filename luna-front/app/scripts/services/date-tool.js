...
angular.module('luna')
  .service('DateTool', function () {
     ...
     * Month        | MMM (name or abbthis.)| MM (2 digits), M (1 or 2 digits)| NNN (abbr)
     ...
     */
    this.check = function (val, pattern) {
      var date = this.stringToDate(val, pattern);
      return date !== 0;
    };
     ...
     *         1    起始日期 < 结束日期
     */
    this.stringCompare = function (string1, pattern1, string2, pattern2) {
      var date1 = this.stringToDate(string1, pattern1), date2 = this.stringToDate(string2, pattern2);
     ...
     */
    this.stringToDate = function (val, pattern) {
      // 输入日期字符串
      val = val + '';
      // 日期格式
     ...
     * @return {Date} 日期对象
     */
    this.distance = function (date, n, unit) {
      var t = 0;
      var strArr;
      var year;
      if (unit === 'y') { //年
        strArr = this.format(date, 'yyyy-MM-dd HH:mm:ss').split('-');
        year = strArr[0] - 0 + n;
        return this.stringToDate(year + '-' + strArr[1] + '-' + strArr[2], 'yyyy-MM-dd HH:mm:ss');
        ...
        return this.stringToDate(year + '-' + leftZero(month) + '-' + day + ' ' + tt[1], 'yyyy-MM-dd HH:mm:ss');
      } else if (unit === 'h') { // 时
        t = 3600 * 1000 * n;
      } else if (unit === 'm') { // 分钟
        t = 60 * 1000 * n;
